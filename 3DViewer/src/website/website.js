...
export class Website {
	...
	InitToolbar() {
		...
		AddButton(this.toolbar, 'open', '从硬盘中打开', [], () => {
			this.OpenFileBrowserDialog();
		});
		AddButton(this.toolbar, 'open_url', '从链接中打开', [], () => {
		...
		AddButton(this.toolbar, 'up_y', '将Y轴设置为上方向向量', ['only_on_model'], () => {
			this.viewer.SetUpVector(Direction.Y, true);
		});
		AddButton(this.toolbar, 'up_z', '将Z轴设置为上方向向量', ['only_on_model'], () => {
			this.viewer.SetUpVector(Direction.Z, true);
		});
		AddButton(this.toolbar, 'flip', '向上翻转矢量', ['only_on_model'], () => {
			this.viewer.FlipUpVector();
		});
		AddSeparator(this.toolbar, ['only_on_model']);
		AddRadioButton(this.toolbar, ['fix_up_on', 'fix_up_off'], ['环绕预览', '自由预览'], 0, ['only_full_width', 'only_on_model'], (buttonIndex) => {
			if (buttonIndex === 0) {
		...
		AddRadioButton(this.toolbar, ['camera_perspective', 'camera_orthographic'], ['透视相机', '正交相机'], 0, ['only_on_model'], (buttonIndex) => {
			if (buttonIndex === 0) {
		...
		AddSeparator(this.toolbar, ['only_full_width', 'only_on_model']);
		AddButton(this.toolbar, 'download', '下载', ['only_full_width', 'only_on_model'], () => {
		...
		AddButton(this.toolbar, 'share', '分享', ['only_full_width', 'only_on_model'], () => {
