...
class ModelExporterUI {
	...
	GenerateParametersUI(parametersDiv) {
		...
		this.visibleOnlySelect = AddSelectItem(parametersDiv, '导出范围', 'ov_last_scope', ['整个模型', '仅可见'], 1);
		this.rotationSelect = AddSelectItem(parametersDiv, '旋转', 'ov_last_rotation', ['不旋转', '-90度', '90度'], 0);
	}
...
class ExportDialog {
	...
	Open(model, viewer) {
		...
		let text = '从下面的列表中选择格式，然后调整所选格式的设置。';
