...
function AddColorPicker(parentDiv, opacity, defaultColor, predefinedColors, onChange) {
	...
	pickr.on('change', (color, source, instance) => {
		...
		onChange(parseInt(rgbaColor[0], 10), parseInt(rgbaColor[1], 10), parseInt(rgbaColor[2], 10), ColorComponentFromFloat(rgbaColor[3]));
	});
	return pickr;
}
	...
	ShowPopup(buttonDiv, shadingType, settings, callbacks) {
		...
		if (shadingType === ShadingType.Phong) {
			...
			if (isPerspective) {
				...
				let backgroundIsEnvMapCheckbox = AddCheckbox(checkboxDiv, 'use_as_background', '显示背景图像', settings.backgroundIsEnvMap, () => {
...
class SettingsModelDisplaySection extends SettingsSection {
	constructor(parentDiv, settings) {
		super(parentDiv, '模型预览设置', settings);
	...
	Init(callbacks) {
		...
		AddDiv(backgroundColorDiv, null, '背景颜色');
		...
		AddDiv(this.environmentMapPhongDiv, null, '背景图像');
		...
		AddDiv(this.environmentMapPbrDiv, null, '环境纹理');
...
class SettingsAppearanceSection extends SettingsSection {
	...
	Init(callbacks) {
		...
		AddDiv(darkModeParameterDiv, null, '暗色模式');
...
export class SidebarSettingsPanel extends SidebarPanel {
	constructor(parentDiv, settings) {
		...
		this.resetToDefaultsButton = AddDiv(this.contentDiv, 'ov_button ov_panel_button outline', '重置为默认设置');
