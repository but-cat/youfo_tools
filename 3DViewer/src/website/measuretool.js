...
export class MeasureTool {
	...
	UpdatePanel() {
		function BlendBackgroundWithPageBackground(backgroundColor) {
			...
			return new RGBColor(parseInt(bgColors[0], 10) * (1.0 - alpha) + backgroundColor.r * alpha, parseInt(bgColors[1], 10) * (1.0 - alpha) + backgroundColor.g * alpha, parseInt(bgColors[2], 10) * (1.0 - alpha) + backgroundColor.b * alpha);
		}
		...
		if (this.markers.length === 0) {
			this.panel.innerHTML = '点击模型放置第一个测量点.';
		} else if (this.markers.length === 1) {
			this.panel.innerHTML = '点击模型放置第二个测量点.';
		} else {
			let calcResult = CalculateMarkerValues(this.markers[0], this.markers[1]);

			if (calcResult.pointsDistance !== null) {
				AddValue(this.panel, 'measure_distance', '直接距离', calcResult.pointsDistance.toFixed(3));
			}
			if (calcResult.parallelFacesDistance !== null) {
				AddValue(this.panel, 'measure_distance_parallel', '平行面的距离', calcResult.parallelFacesDistance.toFixed(3));
			}
			if (calcResult.facesAngle !== null) {
				let degreeValue = calcResult.facesAngle * RadDeg;
				AddValue(this.panel, 'measure_angle', '面的角度', degreeValue.toFixed(1) + '\xB0');
			}
		}
		this.Resize();
	}
