<template>
	<button @click.stop="!filterNode.has(item.name) ? filterNode.add(item.name) : filterNode.delete(item.name)" :class="[filterNode.has(item.name) && '-rotate-90', !item.isSubNode && 'opacity-0']" class="arrow-btn w-3 h-3 mx-2 text-slate-500">
...
import { FileBucket, FileItem, Folde } from "@/browser/store/file/bucket";

const props = defineProps<{
	filterNode: Set<FileItem | Folde>;
	item: FileItem;
}>();
